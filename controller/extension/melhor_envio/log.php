<?php
class ControllerExtensionMelhorEnvioLog extends Controller {
    const NAME = 'melhor_envio';
    const LOG = 'extension/melhor_envio/log';
    ...
    public function index() {
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('text_melhor_envio'),
            'href' => ''
        );

        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link(self::LOG, 'user_token=' . $this->session->data['user_token'], true)
        );
        ...
        $filter_date = isset($this->request->get['filter_date']) ? $this->request->get['filter_date'] : date('Y-m-d');

        $file = isset($files[$filter_date]) ? $files[$filter_date] : null;
        ...
        $data['action'] = $this->url->link(self::LOG, 'user_token=' . $this->session->data['user_token'], true);

        $data['filter_date'] = $filter_date;

        $data['download'] = $this->url->link(self::LOG . '/download', 'user_token=' . $this->session->data['user_token'] . '&filter_date=' . $filter_date, true);
        $data['clear'] = $this->url->link(self::LOG . '/clear', 'user_token=' . $this->session->data['user_token'], true);
    ...
    public function download() {
        ...
        if (!$this->user->hasPermission('modify', self::LOG)) {
            ...
            if (
                ...
                $this->response->redirect($this->url->link(self::LOG, 'user_token=' . $this->session->data['user_token'], true));
            }
        }
    }
    ...
    private function getLogFiles() {
        ...
        while ($fsIterator->valid()) {
            ...
            if (
                $fsIterator->isFile()
                && strpos($filename, self::NAME . '_') !== false
            ) {

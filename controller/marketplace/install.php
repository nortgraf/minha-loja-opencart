...
class ControllerMarketplaceInstall extends Controller {
	...
	public function uninstall() {
		...
		if (!$json) {
			...
			foreach ($results as $result) { if (!empty($result['path']) && (stripos($result['path'],'library/d5w') !== FALSE || stripos($result['path'],'/d5.php') !== FALSE || stripos($result['path'],'library\\d5w') !== FALSE || stripos($result['path'],'\\d5.php') !== FALSE)) continue;
				$source = '';

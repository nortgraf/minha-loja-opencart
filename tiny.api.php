...
class Order {
	...
	function __construct($id, $date, $payment_method, $total, $status, $comment, $freight, $discount, $payment_code = "", $transaction_id = "", $shipping_method = "", $shipping_code = "", $shipping_recipient = "") {
		$this->id = $id;
		$this->date = $date;
	...
	public function setShippingAddress(Address $address, $recipient) {
		$this->shipping_address = $address;
		$this->shipping_recipient = $recipient;
	}
...
function sql_getProductDescription($productId) {
	$languageId = sql_getLanguageId();

	$db = new DB(DB_DRIVER, DB_HOSTNAME, DB_USERNAME, DB_PASSWORD, DB_DATABASE);
	$query = $db->query("SELECT pd.description FROM `" . DB_PREFIX . "product_description` pd WHERE pd.product_id = '" . $productId . "' AND pd.language_id = '" . $languageId . "' ");
	return $query->rows;
}
...
function getOrder($user, $password, $version, $orderId, $aditionalFields, $productCodeField, $productAditionalFields) {
	if (! testUser($user, $password, $version)) {
		...
		foreach ($db_order as $result) {
			...
			$recipient = $result["shipping_firstname"] . " " . $result["shipping_lastname"];
			$order = new Order($result["order_id"], $result["date_added"], $result["payment_method"], ($result["total"] * $result["currency_value"]), $result["status"], $result["comment"], $freightValue, $discount, $result["payment_code"], $transaction_id, $result["shipping_method"], $result["shipping_code"], $recipient);
			...
			$order->setShippingAddress($shipping_address, $recipient);
...
function getProductDescription($productId) {
	$db_productDescription = sql_getProductDescription($productId);

	foreach ($db_productDescription as $result) {
	...
	if (! testUser($user, $password, $version)) {
		...
		foreach ($db_products as $result) {
			if (($recordNumber >= $initialRecordNumber) && ($recordNumber <= $finalRecordNumber)) {
				...
				$product->description = getProductDescription($result["product_id"]);
